...
# --- Background Task Functions (moved to tasks.py) ---
...
class AnalysisTriggerHandler(FileSystemEventHandler):
    ...
    async def _dag_sort_and_enrich(self, uuids: list[str]):
        ...
        sorted_uuids = await topological_sort_uuids(
...
async def scan_codebase(
    ...
    try:
        ...
        if LLM_ENABLED and processed_uuids:
            ...
            async def _worker(sem: asyncio.Semaphore):
                while True:
                    try:
                        ...
                        break
                    try:
                        await process_element_llm(client, uuid, tenant_id, sem)
            ...
            workers = [
                asyncio.create_task(_worker(llm_semaphore))
...
async def trigger_llm_processing(
    ...
    try:
        ...
        sorted_uuids = await topological_sort_uuids(client, tenant_id, uuids_to_process)
        queue: asyncio.Queue[str] = asyncio.Queue()
        ...
        async def _worker(sem: asyncio.Semaphore):
            while True:
                try:
                    ...
                    break
                try:
                    await process_element_llm(client, uuid_item, tenant_id, sem)
                except Exception as w_e:
